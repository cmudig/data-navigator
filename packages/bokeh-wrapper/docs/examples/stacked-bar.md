...
<dn-commands-table id="stacked-commands-table"></dn-commands-table>
</div>
...
onMounted(async () => {
  ...
  const cmdTable = document.getElementById('stacked-commands-table');
  if (cmdTable) {
    cmdTable.commands = [
      { label: 'Enter navigation area button', description: 'Start keyboard navigation' },
      { label: '← →', description: 'Navigate between years (or browsers at the deepest level)' },
      { label: '↑ ↓', description: 'Navigate between browsers (or years at the deepest level)' },
      { label: 'Enter', description: 'Drill in' },
      { label: 'W', description: 'Go up to year level' },
      { label: 'J', description: 'Go up to browser level' },
